...
public class Nation {
	...
	void debug(){
		...
		p.text("number of wars: "+currentWars.size(), pos.x, pos.y);
		p.text("victory behaviour: "+victoryBehaviour, pos.x, pos.y+10);
		p.text("wealth: "+totalWealth, pos.x, pos.y+30);
		p.text("war: "+atWar, pos.x, pos.y + 20);
		p.text("vanquished: "+isVanquished, pos.x, pos.y - 20);
		p.text("enemies: "+enemies.size(), pos.x+20, pos.y+50);
		p.text("isRuined: "+isRuined, pos.x+20, pos.y+70);
	}
	...
	void victoryBehaviour(Nation v, Nation e){
		...
		if(victoryBehaviour == 1.5f){
			p.println("easy victory behaviour");
			...
			p.println("medium victory behaviour");
			...
			//				}
			//			}
			p.println("death to the vanquished");
	...
	void update(){
		...
		if(enemies.size() > 0)
			totalWealth -= 0.05f * enemies.size();
		
		
		for(int i = 0; i < enemies.size(); i++){
			Nation e = enemies.get(i);
			if(e.totalWealth < wealthStopWar && !this.vanquished.contains(e) && !e.isVanquished){
	...
	void war(){
		...
		for(int i = 0; i < PolSys.others.size(); i++){
			...
			}else if(PApplet.abs(this.culturalHomogeneity - n.culturalHomogeneity) < culturalRequirementWar*0.9f || this.isRuined || n.isRuined){
				
				for(int j = 0; j < PolSys.wars.size(); j++){
					War w = PolSys.wars.get(j);

					if((w.n1 == this && w.n2 == n) || (w.n1 == n) && (w.n2 == this)){
						p.println("war "+w+" removed because of culture or isRuined?@"+p.frameCount);
