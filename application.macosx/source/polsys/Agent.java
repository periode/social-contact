...
public class Agent extends ProcessingObject {
	...
	Agent(float x, float y, float rad, int connections, float speed1, float speed2, float distance, float formingStableRelationships, float meetingOthers, float violence, float resourceSeek, PApplet p) {
		...
		companions = new ArrayList<Agent>();
		...
		if (r > 0.7f) {
			this.connections = connections + (int)(p.random(1, 3));
		} else if(r > 0.85f){
			this.connections = connections - (int)(p.random(1, 2));
		}else{
			this.connections = connections;
		}
		originalNumberOfConnections = connections;
		this.connecMax = this.connections;
		float wealthMod = PApplet.map(wealthAccumulation, 0, 500, 0, 1);
		power = connecMax*wealthMod;
		connecPower = connections;
		connecLove = 1;
		connecFriendship = connections + (int)p.random(1, 6);
		...
		timerDeathMax = p.random(220, 445)*1000;
		timerDeath = timerDeathMax;
		lifeSpan = p.random(208, 255)*1000;
		birth = p.millis();
		agingRate = 0.1f;
		...
		distanceFriendship = p.width*0.15f;
		distanceFriendshipSeparate = distanceFriendship * 1.1f;
	...
	void update(){
		...
		if(PolSys.inGame1){
			...
			connections = connecMax-currentConnections.size();
		}
		...
		if (formingStableRelationships == 1.5f) {
			if (connections == (int)(connecMax*0.0f)){
				moveCoeff = 0.0f;
			}else if (connections <= (int)(connecMax*0.25f)){
				moveCoeff = 0.0f;
			}else if (connections <= (int)(connecMax*0.5f)){
				moveCoeff = 0.4f;
			}else if (connections <= (int)(connecMax*0.75f)){
				moveCoeff = 0.8f;
			}else{
				moveCoeff = 0.8f;
			}
	...
	void debug(){
		...
		//p.text("c: "+connections, pos.x-rad, pos.y+rad);
		//p.text("c max: "+connecMax, pos.x+rad, pos.y-rad);
	...
	void display() {
		...
		if ((p.millis() - startTimeMovement >= timerMovement) && connections >= connecMax-1) { //only change speed and type when you are alone
		...
		if(PolSys.inGame1){ //this is the dying/aging condition for the first part
			...
			if(connections == connecMax || allDeceased){
				isAlone = true;
			}else{
				isAlone = false;
			}
	...
	public String computeThought(int stage, float r){
		...
		if(stage == 1){
			...
			if(isKiller && connections == connecMax-1 && !tLoneKiller){
	...
	void exertPower(){
		for(int i = 0; i < agentsPower.size(); i++){
			...
			if(a.isAlive && a.isSettled && a.pos.y < PolSys.rectBorderH*0.95f && a.pos.y > PolSys.rectBorderY*2.0f){
	...
	void connect() {
		if(connections == connecMax) lastTimeWithoutRelationship = (p.millis() - startTimeWithoutRelationship)/1000.0f;
			...
			if (PApplet.dist(this.pos.x, this.pos.y, PolSys.agents[i].pos.x, PolSys.agents[i].pos.y) < this.distance && this.connections > 0 && PolSys.agents[i].connections > 0 && this.arrived && PolSys.agents[i].arrived) { //you are within the distance, and still have open connections left
				...
				if (!foundConnection) { //if you haven't found a connection between the two agents, ...
					if (this.isAlive && (PolSys.agents[i].isAlive || PolSys.rememberDead > 1.0f) && this.hasGrave && PolSys.agents[i].hasGrave) { // ...and if the two agents still have connections left
						...
						if(PolSys.agents[i].connections == PolSys.agents[i].connecMax){ //the time that has elapsed since the last time it broke apart
						...
						this.connections--;
						PolSys.agents[i].connections--;
						
						//this.companions.add(PolSys.agents[i]);
						//PolSys.agents[i].companions.add(this);
				...
				for (int j = 0; j < PolSys.connections.size (); j++) {
					...
					if (c.a2 == PolSys.agents[i] && c.a1 == this || c.a1 == PolSys.agents[i] && c.a2 == this) { //if this is the connection between the two agents
						...
						if(this.cluster != null){
							this.cluster.removeAgent(PolSys.agents[i]);
						}
						if(PolSys.agents[i].cluster != null){
							PolSys.agents[i].cluster.removeAgent(this);
						}
						...
						if(this.connections == this.connecMax-1){//if this is your last connection, set the start time for a new record (hopefully not)
							this.startTimeWithoutRelationship = p.millis();
						}

						if(PolSys.agents[i].connections == PolSys.agents[i].connecMax-1){ //same deal if it's the case for the other agent
				...
				//this part is where i remember the dead, so its dependent on PolSys.rememberDead, but it might create weird stuff regarding the dead's cluster
				...
				for (int j = 0; j < PolSys.connections.size (); j++) {
					...
					if (c.a2 == PolSys.agents[i] && c.a1 == this || c.a1 == PolSys.agents[i] && c.a2 == this) { //if this is the connection between the two agents
						...
						if(this.isAlive && this.connections == this.connecMax-1){//if this is your last connection, set the start time for a new record (hopefully not)
							this.startTimeWithoutRelationship = p.millis();
						}
		...
		if(PolSys.inGame1){
			for (int i = 0; i < PolSys.agents.length; i++) {
				if (PApplet.dist(this.pos.x, this.pos.y, PolSys.agents[i].pos.x, PolSys.agents[i].pos.y) < rad && PolSys.agents[i] != this && PolSys.agents[i].isAlive && this.arrived && PolSys.agents[i].arrived) {
					...
					//can disturb
					if (meetingOthers == 1.0f && this.connections == connecMax && PolSys.agents[i].connections <= 1) { //the current agent is the outsider and they could be trusted
						float r = p.random(1);
						if(r < 0.4f){
							this.hasDisturbed += 1;
							this.isKiller = true;
					...
					if (meetingOthers == 1.5f && this.connections >= connecMax-1 && PolSys.agents[i].connections <= 1) { //both die
					...
					// combat stuff
					if (this.connections == connecMax-1 && PolSys.agents[i].connections == PolSys.agents[i].connecMax-1) { //if they are both alone (they only have one connection)
					...
					}else if(PolSys.agents[i].connections == 0){
	...
	void makeSound(int type){
		if(type == 0){//connection
			//int numberOfHarmonics = (int)PApplet.map(connections, 0, connecMax, 1, 10); //this should be mapped to total connections made already
