...
public class Agent extends ProcessingObject {
	...
	ArrayList<Connection> currentConnections;
		  ...
		  boolean canFormStableRelationships;
		  boolean canDisturb;
		  boolean canProtect;
		  ...
		  static float cultSimRequirement;
		  ...
		  Agent(float x, float y, float rad, int connections, float speed1, float speed2, float distance, boolean right1, boolean right2, boolean right3, float violence, float resourceSeek, PApplet p) {
		    this.p = p;
		    ...
		    canFormStableRelationships = right1;
		    canDisturb = right2;
		    canProtect = right3;
		    
		    this.violence = violence + p.random(1)/10;
			...
			powerModifier = 5.0f;
			maxPowerForce = 0.01f * powerModifier;
			powerRatio = 0.1f;
			powerExerted = 0.0f;
			powerFaced = 0.0f;
		  ...
		  void update(){
			  if(PolSys.inGame1){
				  ...
				  for(int i = 0; i < PolSys.resources.size(); i++){
					  ...
					  if(PApplet.dist(pos.x, pos.y, r.pos.x, r.pos.y) < distanceResource + r.rad && r.rad > r.maxRad*0.1f){
			  ...
			  if(PolSys.inGame2){
				  ...
				  if(currentConnectionsLove.size() > 0){
					timeInLove = p.millis()-currentConnectionsLove.get(0).creationTime;
				  }
				  ...
				  alpha2 = PApplet.map((p.millis()-startTimeDeath2), 0, lifeSpan, 255, 0);
			  }
			  ...
			  if(isStopped){			    
			    ...
			    //stable relationships = stop moving when you have one connection left
			    if (canFormStableRelationships) {
			      if (connec == 0) moveCoeff = 0.0f;
			      if (connec == 1) moveCoeff = 0.0f;
			      if (connec == 2) moveCoeff = 0.4f;
			      if (connec == 3) moveCoeff = 0.8f;
			    } else {
			      if (connec == 0) moveCoeff = 0.0f;
			      if (connec == 1) moveCoeff = 0.2f;
			      if (connec == 2) moveCoeff = 0.7f;
			      if (connec == 3) moveCoeff = 1;
			    }
		  ...
		  void seek(PVector target, float limitForce, float coeff){
			  ...
			  desired.normalize();
			  if(PolSys.inGame2){
				  if(isSettled) desired.mult(moveCoeff);
				  //if(!isSettled) desired.mult(moveCoeff*3.0f);
			  }else{
		  ...
		  void debug(){
			  ...
			  p.fill(0);
			  //p.text(""+this.cluster.hashCode() + " + " + this.cluster.agentsInside.size(), x+rad, y-rad);
			  //p.text(""+this.cluster.agentsInside.contains(this), x+rad, y+rad*2);
			  ...
			  //p.text("coToDead "+isConnectedToDead, pos.x-rad, pos.y+rad);
			  ...
			  //p.text("powerFaced: "+powerFaced, pos.x, pos.y+rad+rad);
			  //p.text("powerExerted: "+powerExerted, pos.x, pos.y+rad);
		  ...
		  void display() {
			  ...
			  if(PolSys.inGame1){
				  ...
				  p.strokeWeight(2);
				    for (int i = 0; i < connec; i++) {
				    	p.ellipse(pos.x, pos.y, (i+1)*3, (i+1)*3);
				    }
		    ...
		    if ((p.millis() - startTimeMovement >= timerMovement) && connec >= connecMax-1) { //only change speed and direction when you are alone
		    ...
		    if(PolSys.inGame1){ //this is the dying/ageing condition for the first part
			    if(connec == connecMax || (isKiller && connec == connecMax-1)){
			    	isAlone = true;
			    }else{
			    	isAlone = false;
			    }
			  ...
			  for(int i = 0; i < agentsPower.size(); i++){
				  ...
				  if(a.isAlive && a.isSettled){
					  ...
					  if(diff > 0){//if this one is the most powerful, exert power
						  ...
						  //do not exert power
						  //actually, nothing should be done in the else statement else the force will be applied twice innit
					  }
				  ...
				  if(a.isAlive && !a.isSettled){
					  if(PApplet.dist(this.pos.x, this.pos.y, a.pos.x, a.pos.y) > personalSpaceFriendship){ //this is for calculating vectors
						  ...
						  for(int j = 0; j < culture.length; j++){
							  culturalSimilarity += PApplet.abs(this.culture[j] - a.culture[j]); //the higher the number, the more similar
						  }
						  
						  if(culturalSimilarity < 1){
							  culturalSimilarity = maxFriendshipForce;
						  }else{
							  culturalSimilarity = 1/culturalSimilarity;
						  }
						  
						  p.fill(255, 0, 0);
						  //p.textSize(12);
						  //p.text("culSim "+culturalSimilarity, pos.x, pos.y+rad*2+(i*10));
						  PVector friendship = PVector.sub(a.pos, this.pos);
						  friendship.normalize(); //now we have a force pointing at the other friend;
						  //friendship.y = 0; //now the force is only on the x axis MAYBE I DON'T NEED THAT
						  friendship.mult(culturalSimilarity*0.02f);
		  ...
		  void reproduce(){
			  if(isOfAge && canReproduce && agentsLove.size() > 0 && !isSterile){
				  ...
				  ConnectionFriendship bond = new ConnectionFriendship();
					  ...
					  if(culturalSimilarity < maxSimilarity && a.isOfAge && a.canReproduce && a.isAlive){
				  	...
				  	if(mate != null && mate.canReproduce && !mate.isSterile && p.millis() - bond.creationTime > this.reproduceSpanFriendship && p.millis() - bond.creationTime > mate.reproduceSpanFriendship && mate != parent1 && mate != parent2 && this.numberOfChildren > 0 && mate.numberOfChildren > 0){
						  ...
						  boolean r1 = canFormStableRelationships;
						  boolean r2 = canDisturb;
						  boolean r3 = canProtect;
						  float v = (this.violence+mate.violence)/2;
						  float reS = this.resourceSeek;
						  
						  //this is the result
						  PApplet.println("someone is born @ "+p.frameCount);
						  Agent child = new Agent(x, y, r, c, -2, 2, d, r1, r2, r3, v, reS, this.p);
						  child.parent1 = this;
						  child.parent2 = mate;
		    ...
		    for (int i = 0; i < PolSys.agents.length; i++) {
		      if (PApplet.dist(this.pos.x, this.pos.y, PolSys.agents[i].pos.x, PolSys.agents[i].pos.y) < this.distance && PolSys.agents[i] != this && connec > 0 && PolSys.agents[i].connec > 0 && this.arrived && PolSys.agents[i].arrived) { //you are within the distance, and still have open connections left
		        ...
		        if (!foundConnection) { //if you haven't found a connection between the two agents, ...
		          if (this.isAlive && PolSys.agents[i].isAlive) { // ...and if the two agents still have connections left
		      ...
		      } else if (PApplet.dist(this.pos.x, this.pos.y, PolSys.agents[i].pos.x, PolSys.agents[i].pos.y) > this.distance) { //if they are too far apart
		        for (int j = 0; j < PolSys.connections.size (); j++) {
		          ...
		          if (c.a2 == PolSys.agents[i] && c.a1 == this || c.a1 == PolSys.agents[i] && c.a2 == this) { //if this is the connection between the two agents
		            ...
		            if (!PolSys.agents[i]._dfsMarked) {
		              ...
		              //agents[i].cluster = cl;
		        /*
		              for (int n = 0; cluster !=null && n < clTemp.agentsInside.size (); n++) {

		                Agent agent = clTemp.agentsInside.get(n);
		                
		                if (!agent._dfsMarked) {
		                  if(!cl.agentsInside.contains(agent)) cl.agentsInside.add(agent);
		  ...
		  void connectLove(){
			  ...
			  for(int i = 0; i < PolSys.community.size(); i++){
				  ...
				  //TODO this part of the code is where you must chose between the person that is the most similar to you when creating a connection
				  ...
				  }else if(PApplet.dist(this.pos.x, this.pos.y, a.pos.x, a.pos.y) > this.distanceLoveSeparate){
					  for(int j = 0; j < PolSys.connectionsLove.size(); j++){
						  ConnectionLove cL = PolSys.connectionsLove.get(j);
		  ...
		  void connectPower(){
			  for(int i = 0; i < PolSys.community.size(); i++){
				  Agent a = PolSys.community.get(i);
				  if(PApplet.dist(this.pos.x, this.pos.y, a.pos.x, a.pos.y) < this.distancePower && this != a && this.connecPower > 0 && a.connecPower > 0){
					  boolean foundConnection = false;
		  ...
		  void connectFriendship(){
			  for(int i = 0; i < PolSys.community.size(); i++){
				  ...
				  if(PApplet.dist(this.pos.x, this.pos.y, a.pos.x, a.pos.y) < this.distanceFriendship && this != a && this.connecFriendship > 0 && a.connecFriendship > 0 && cultSim < cultSimRequirement){
					  boolean foundConnection = false;
			  ...
			  if(PolSys.inGame1){
				  for (int i = 0; i < PolSys.agents.length; i++) {
				      if (PApplet.dist(this.pos.x, this.pos.y, PolSys.agents[i].pos.x, PolSys.agents[i].pos.y) < rad && PolSys.agents[i] != this && PolSys.agents[i].isAlive && this.arrived && PolSys.agents[i].arrived) {
				        ...
				        //can disturb
				        if (canDisturb) {
				          if (this.connec >= connecMax-1 && PolSys.agents[i].connec == 0) { //when this agent hits another one, then he forces him to have more connections.
				            this.hasDisturbed += 1;
				            this.isKiller = true;
				        ...
				        //can protect
				        if (canProtect) {
				          if (this.connec == 0 && PolSys.agents[i].connec == connecMax) { //when this gets by another full agent, he fights back
				        	PApplet.println("protected");
				        	PolSys.agents[i].cluster.buryAgent(PolSys.agents[i]);
				            PolSys.agents[i].colDeath = p.color(0, 255, 0);
				            PolSys.agents[i].isKilled = true;
				            this.hasProtected += 1;
				        ...
				        if (this.connec == connecMax-1 && PolSys.agents[i].connec == PolSys.agents[i].connecMax-1) { //if they are both alone (they only have one connection)
				          ...
				          if (r < violence) { //they have a x percent chance of fighting
				        ...
				        bounce = PolSys.agents[i].velocity.get();
				        bounce.mult(-2.0f);
				        PolSys.agents[i].applyForce(bounce);
				      }
				    }
			  }
		  ...
		  void speak(){
			  if(canTalk){
				  ...
				  for(int i = 0; i < numHarmonics; i++){
					  wp[i] = new WavePlayer(PolSys.ac, baseFreq+(baseFreq*PApplet.map(this.culture[2], 0.0f, 1.0f, 0.0f, 100.0f)), Buffer.SINE); //freq depends on social environment
					  e[i] = new Envelope(PolSys.ac, 0.0f);
					  g[i] = new Gain(PolSys.ac, 1, e[i]);
					  g[i].addInput(wp[i]);
					  float riseValueMax = 0.05f+0.2f*this.culture[1]+p.random(0.01f); //loudness depends on power
					  e[i].addSegment(riseValueMax, 25.0f+culture[0]*25.0f);//attack depends on the space
		  ...
		  void makeSound(int type){
			  if(type == 0){//connection
				  ...
				  for(int i = 0; i < numberOfHarmonics; i++){
					  ...
					  //TODO normalize
					  //p.println("numh: "+numberOfHarmonics);
					  //float riseValue = 1/numberOfHarmonics; // (0.1f+gainAddition)*
					  float riseValueMax = 0.0025f*gainAddition;
					  //p.println("rise: "+riseValueMax);
				  ...
				  for(int i = 0; i < numberOfHarmonics; i++){
					  ...
					  e[i].addSegment(0.0f, 1500.0f, new KillTrigger(g[i])); //TODO maybe because I don't kill the envelope, it might fuck up and over time?
				  }
