...
public class Nation {
	...
	Nation(PVector pos, float rateOfFriendship, float rateOfPower, float rateOfLove, float population, float culturalHomogeneity, float canFight, boolean canProtect, int connections, float craveForWealth, float wealth, boolean isNation, PApplet p){
		this.p = p;
		this.pos = pos;
		this.origPos = pos;
		...
		this.culturalHomogeneity = culturalHomogeneity; //TODO normalize it, so that it gets to sth like "5+cultHom", where cultHom is between -x and +x
		...
		allianceMovementLerp = 0.01f;
		
		culturalRequirementWar = 4.0f;
	...
	void debug(){
		...
		//p.text("wealth: "+totalWealth, pos.x, pos.y+20);
		...
		//p.text(victoryBehaviour, pos.x+20, pos.y+30);
	}
	...
	void victoryBehaviour(Nation e){
		p.println("victor"+victoryBehaviour);
		if(victoryBehaviour == 0.5f){
			...
			for(int j = 0; j < currentWars.size(); j++){
				...
				if((w.n1 == this && w.n2 == e) || (w.n2 == this && w.n1 == e)){
					...
					targetPos = new PVector((e.pos.x+this.pos.x)*0.5f, (e.pos.y+this.pos.y)*0.5f);
					moveLerpMax = 0.2f; //moves the nation half-way
				}
			}
		}
	}
	...
	void update(){
		...
		this.col = (int)PApplet.map(culturalHomogeneity, 0.0f, 10.0f, 0.0f, 360.0f);
		for(int i = 0; i < allies.size(); i++){
			Nation a = allies.get(i);
			this.pos = PVector.lerp(origPos, a.pos, allianceMovementLerp);
		}
			 ...
			 for(int i = 0; i < this.allies.size(); i++){
				 ...
				 for(int j = 0; j < a.enemies.size(); j++){
					 ...
					 if(allianceTrust == 0.5f){
						 ...
						 if(!this.enemies.contains(e)){
							 War w = new War(this, e, p);
							 PolSys.wars.add(w);
							 this.enemies.add(e);
							  this.currentWars.add(w);
							  e.currentWars.add(w);
							  if(!e.enemies.contains(this)) e.enemies.add(this);
						 }
	...
	void war(){
		//if cultural difference is too bad, then war
		  for(int i = 0; i < PolSys.others.size(); i++){
			  ...
			  if(PApplet.dist(this.pos.x, this.pos.y, n.pos.x, n.pos.y) < this.distanceWar && this != n && PApplet.abs(this.culturalHomogeneity-n.culturalHomogeneity)  > culturalRequirementWar && n.totalWealth > wealthStopWar*1.1f){
				  boolean found = false;
			  ...
			  }else if(PApplet.abs(this.culturalHomogeneity - n.culturalHomogeneity) > culturalRequirementWar){
				  for(int j = 0; j < PolSys.wars.size(); j++){
					  War w = PolSys.wars.get(j);
