	    ...
	    for (int i  = 0; i < PolSys.agents.length; i++) {
	      if (this.pos.x+size/2 > PolSys.agents[i].pos.x - PolSys.agents[i].rad/2 && this.pos.x-size/2 < PolSys.agents[i].pos.x + PolSys.agents[i].rad/2 && this.pos.y+size/2 > PolSys.agents[i].pos.y - PolSys.agents[i].rad/2 && this.pos.y-size/2 < PolSys.agents[i].pos.y - PolSys.agents[i].rad/2) { //if they coll
	        if (PolSys.agents[i].isAlive  && PolSys.agents[i].arrived) {
	          ...
	          if (hungerDuration <= 0 && !PolSys.agents[i].isHunter && !isDomesticated) { //if the beast is hungry and untamed

	              float r = p.random(1);

	            if (r > 0.3f) { //70% chance the agent is dead

	            	PolSys.agents[i].isAlive = false; //other is dead
	             	PolSys.agents[i].isHunted = true;
	             	//PolSys.sfx_hunted.trigger();
	             	...
	             	for (int j = 0; j < PolSys.connections.size (); j++) {
	             		...
	             		if(!PolSys.canRememberFallen){ //if they don't have the right to remember the fallen
		             		...
		             		if (c.a1 == PolSys.agents[i]){
		             			...
		             			//this should not happen because it's removed on line 110
		             			if(c.a2.cluster.agentsInside.contains(PolSys.agents[i])){
		             				PApplet.println("this should not happen, see comment 123");
		             				c.a2.cluster.agentsInside.remove(PolSys.agents[i]);
		             			}
		             		...
		             		if(c.a2 == PolSys.agents[i]){
		             			
		             			c.a1.connec++;
		             			
		             			c.a1.isConnectedToDead = true;
		             			
		             			c.a1.cluster.buryAgent(PolSys.agents[i]); //remove any reference to it, and set its cluster to null
		             			
		             			//same deal as in 123
		             			if(c.a1.cluster.agentsInside.contains(PolSys.agents[i])){
		             				PApplet.println("this should not happen v2.0");
